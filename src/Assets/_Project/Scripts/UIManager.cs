...
public class UIManager : MonoBehaviour
{
    ...
    public void EndGame(float seconds = 3)
    {
        StartCoroutine(EndGameEnum(seconds));
    }

    IEnumerator EndGameEnum(float seconds)
    {
        yield return new WaitForSecondsRealtime(seconds);
        foreach (var item in FindObjectsOfType<Chicken>())
        {
            Destroy(item.gameObject);
        }
        FindObjectOfType<DialogueRunner>().StartDialogue("Leave");
    }
    ...
    IEnumerator ThanksForPlayingEnum()
    {
